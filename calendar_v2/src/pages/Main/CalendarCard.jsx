...
export default function CalendarCard({calendar}) {
    ...
    return (
        <div 
        onClick={handleClick}
        className={` ${loading? 'scale-105 active:scale-105' : ''} overflow-hidden w-40 h-64 rounded-md border custom-border-accentMediumStrong select-none m-2 transition-all active:scale-110 hover:scale-105 hover:shadow-bottom-right duration-200 ease-in-out hover:shadow-accentLight cursor-pointer`}>
            ...
            <section className="no-scrollbar relative h-full text-custom-text-baseColor break-words text-sm p-2 overflow-auto">
                ...
                <AnimatedContainer animation='reverseCurtainVariant' className='absolute inset-0 background-accentMediumStrong' >
                    <LoadingIcon theme='loadingIconReverse'/>
                </AnimatedContainer> 
                    : false}
            </section>
        </div>
